...
import com.github.monaboiste.fairshare.graphs.fixture.userjourney.Product.ProductType
...
record UserJourney(UserJourneyId userJourneyId, Graph<State, Condition> graph, State currentState) {
    ...
    Set<CustomerPath> waysToAchieve(ProductType productType) {
        ...
        return statesWithProduct.stream()
                .flatMap(targetState ->
                        new AllDirectedPaths<>(graph).getAllPaths(currentState(), targetState, true, null).stream())
    ...
    Optional<CustomerPath> optimizedWayToAchieve(ProductType productType, Function<Condition, Double> weightFunction) {
        return waysToAchieve(productType).stream().min(Comparator.comparingDouble((CustomerPath path) -> path.weight(weightFunction)))
    }
}
...
class Builder {
    ...
    UserJourney build() {
        return new UserJourney(userJourneyId, graph, requireNonNull(currentState, "currentState"))
    }
}
