...
class CompositeFunctionCalculatorScenarioSpec extends Specification {
    ...
    def "parking price depends on time of day"() {
        ...
        PricingResult dayPrice = composite.calculate(Parameters.of("parkingTime", LocalTime.of(15, 0)))
        PricingResult nightPrice = composite.calculate(Parameters.of("parkingTime", LocalTime.of(23, 0)))

        then:
        new BigDecimal("5.00") == dayPrice.money().value()
        new BigDecimal("2.00") == nightPrice.money().value()
    }
    ...
    def "volume discounts are applied by quantity"() {
        ...
        PricingResult smallPrice = composite.calculate(Parameters.of("quantity", new BigDecimal("5")))
        PricingResult mediumPrice = composite.calculate(Parameters.of("quantity", new BigDecimal("25")))
        PricingResult largePrice = composite.calculate(Parameters.of("quantity", new BigDecimal("100")))

        then:
        new BigDecimal("10.00") == smallPrice.money().value()
        new BigDecimal("8.00") == mediumPrice.money().value()
        new BigDecimal("6.00") == largePrice.money().value()
    }
    ...
    def "shipping cost is determined by package weight"() {
        ...
        PricingResult tinyPrice = composite.calculate(Parameters.of("weight", new BigDecimal("0.5")))
        PricingResult smallPrice = composite.calculate(Parameters.of("weight", new BigDecimal("3.5")))
        PricingResult largePrice = composite.calculate(Parameters.of("weight", new BigDecimal("15")))

        then:
        new BigDecimal("12.00") == tinyPrice.money().value()
        new BigDecimal("18.00") == smallPrice.money().value()
        new BigDecimal("45.00") == largePrice.money().value()
    }
    ...
    def "bar pricing applies a happy-hour discount in the early evening"() {
        ...
        PricingResult afternoonPrice = composite.calculate(Parameters.of("orderTime", LocalTime.of(14, 0)))
        PricingResult happyPrice = composite.calculate(Parameters.of("orderTime", LocalTime.of(18, 0)))
        PricingResult eveningPrice = composite.calculate(Parameters.of("orderTime", LocalTime.of(21, 0)))

        then:
        new BigDecimal("25.00") == afternoonPrice.money().value()
        new BigDecimal("15.00") == happyPrice.money().value()
        new BigDecimal("25.00") == eveningPrice.money().value()
    }
    ...
    def "transfer fees are applied based on transfer amount"() {
        ...
        PricingResult smallFee = composite.calculate(Parameters.of("amount", new BigDecimal("50")))
        PricingResult mediumFee = composite.calculate(Parameters.of("amount", new BigDecimal("500")))
        PricingResult largeFee = composite.calculate(Parameters.of("amount", new BigDecimal("5000")))

        then:
        BigDecimal.ZERO == smallFee.money().value()
        new BigDecimal("2.00") == mediumFee.money().value()
        new BigDecimal("5.00") == largeFee.money().value()
    }

    private CalculatorId addFixedCalculator(String name, Money amount) {
        Calculator calculator = Calculators.fixed(name, amount)
