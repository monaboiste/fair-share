...
class PricingFacadeSpec extends Specification {
    ...
    def "available calculators includes the pre-registered default calculators"() {
        ...
        and:
        assert views.size() == 2
        assert views.any { it.name() == "simple-fixed-20" }
        assert views.any { it.name() == "simple-interest-6" }
    }
    def "available calculators grows when a new calculator is registered"() {
        ...
        List<CalculatorView> views = facade.availableCalculators()
        and:
        assert views.size() == before + 1
        assert views.any { it.name() == "extra-fixed" }
    }
    def "available calculators includes the correct type for each entry"() {
        ...
        and:
        assert view != null
        assert view.type() == CalculatorType.SIMPLE_FIXED
        assert view.description() != null
        assert view.calculatorId() != null
    }
    def "list calculators with descriptions groups by calculator type"() {
        ...
        and:
        assert grouped.get(CalculatorType.SIMPLE_FIXED).size() >= 2
        assert grouped.get(CalculatorType.PERCENTAGE).size() >= 1
    }
    ...
    def "addCalculator returns the created calculator"() {
        ...
        and:
        assert calc != null
        assert calc.name() == "my-calc"
        assert calc.getType() == CalculatorType.SIMPLE_FIXED
    }
